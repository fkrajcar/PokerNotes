...
function PMA_getPlugin(
    ...
    if (is_file($plugins_dir . $file)) {
        ...
        $fqnClass =
            'PMA\\' . str_replace('/', '\\', $plugins_dir) . $class_name;
...
function PMA_getPlugins($plugin_type, $plugins_dir, $plugin_param)
{
    ...
    $class_type =
        mb_strtoupper($plugin_type[0], 'UTF-8') .
    ...
    while ($file = @readdir($handle)) {
        ...
        if (
            ...
            preg_match('@^' . $class_type . '([^\.]+)\.php$@i', $file, $matches)
        ) {
...
function PMA_pluginGetChoice($section, $name, &$list, $cfgname = null)
{
    ...
    foreach ($list as $plugin) {
        ...
        $plugin_name = mb_strtolower(
            mb_substr($plugin_name, mb_strlen($section))
        );
        $ret .= '<option';
        // If the form is being repopulated using $_GET data, that is priority
        if (
            (isset($_GET[$name]) && $plugin_name == $_GET[$name]) ||
        ...
        $hidden .=
            '<input type="hidden" id="force_file_' . $plugin_name . '" value="';
...
function PMA_pluginGetOneOption(
    ...
    if (!$is_subgroup) {
        ...
        if (mb_strpos(get_class($propertyGroup), "PropertyItem")) {
            ...
            // for main groups
            $ret .=
                '<div class="export_sub_options" id="' .
                $plugin_name .
                '_' .
